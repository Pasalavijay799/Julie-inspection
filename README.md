...
### Direct Ethernet Gripper (OnRobot 2FG7)
The repository now includes a direct Ethernet gripper node (`onrobot_2fg7_driver.py`) using Modbus/TCP.
...
2. Launch arm + gripper together (real robot):
```bash
ros2 launch jaka_planner moveit_server.launch.py ip:=192.168.1.50 model:=zu5 use_gripper:=true gripper_ip:=192.168.1.100
```

3. Quick open/close without MoveIt (service on the gripper node):
```bash
ros2 service call /onrobot_2fg7_driver/set_gripper std_srvs/srv/SetBool "{data: true}"   # open
ros2 service call /onrobot_2fg7_driver/set_gripper std_srvs/srv/SetBool "{data: false}"  # close
```

4. If your gripper uses different Modbus register mapping, adjust:
```bash
src/jaka_driver/config/onrobot_2fg7_modbus.yaml
```
