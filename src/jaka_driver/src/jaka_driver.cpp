...
void tool_position_callback(
  ...
  tcp_position.tran.x = 0; tcp_position.tran.y = 0; tcp_position.tran.z = 0;
  tcp_position.rpy.rx = 0; tcp_position.rpy.ry = 0; tcp_position.rpy.rz = 0;
  RotMatrix rot;
  Rpy rpy;
  // robot.get_robot_status(&robotstatus);
  int ret = robot.get_tcp_position(&tcp_position);
  if (ret != 0) return;
...
void joint_position_callback(
  ...
  for (int i=0; i<6; i++) joint_pos.jVal[i] = 0.0;
  // robot.get_robot_status(&robotstatus);
  int ret = robot.get_joint_position(&joint_pos);
  if (ret != 0) return;
...
int main(int argc, char *argv[]) {
  ...
  string default_ip = "192.168.1.50";
